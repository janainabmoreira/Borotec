...
import {
  Plus, Check, Camera, Monitor, Plug2, Droplets,
...
type FilterKey = keyof Omit<PocosProduct, 'id' | 'name' | 'description' | 'image' | 'specLabels'>;
...
const filterConfig: { key: FilterKey; label: string; options: string[] }[] = [
  { key: 'cable',  label: 'Comprimento', options: ['50m', '100m']     },
...
const ProductCard = ({ product }: { product: PocosProduct }) => {
  ...
  return (
    <div className="group bg-card rounded-2xl overflow-hidden border border-border shadow-card hover:border-accent/40 hover:-translate-y-1 transition-all duration-300">
      <Link to={`/linha-p/${product.id}`} className="block relative overflow-hidden aspect-[4/3] bg-secondary">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
        ...
        <div className="absolute top-3 left-3">
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-teal-500/90 backdrop-blur-sm text-white text-[10px] font-bold font-heading rounded-full uppercase tracking-wide">
            <IconPoco className="w-3 h-3" /> Linha P
          </span>
        </div>
      </Link>

      <div className="p-5">
        <Link to={`/linha-p/${product.id}`}>
...
const LinePocos = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Câmeras para Poços e Subaquático | Linha P | BOROTEC Industrial</title>
        <meta name="description" content="Câmeras submersíveis para inspeção de poços artesianos, reservatórios e ambientes subaquáticos. IP68, cabo de até 100m, câmera Full HD com LEDs potentes." />
        <link rel="canonical" href="https://borotec.com.br/linha-p" />
        <meta property="og:title" content="Câmeras para Poços e Subaquático | BOROTEC Industrial" />
        <meta property="og:description" content="Inspeção submersível de poços artesianos e reservatórios com câmeras IP68 e cabo de 100m." />
        <meta property="og:url" content="https://borotec.com.br/linha-p" />
          ...
          <section className="relative bg-charcoal py-12 md:py-20 overflow-hidden">
            ...
            <div className="relative container-wide mx-auto px-4 md:px-8">
              <nav className="flex items-center gap-2 text-sm text-primary-foreground/50 mb-6">
                ...
                <span>/</span>
                <span className="text-cyan">Poços e Subaquático</span>
              </nav>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-cyan/20 rounded-xl flex items-center justify-center">
                      <IconPoco className="w-5 h-5 text-cyan" />
                    </div>
                    <span className="inline-flex items-center px-3 py-1 bg-teal-500/15 text-teal-400 text-xs font-semibold font-body rounded-full">
                      Linha P
                    </span>
                  </div>
                  <h1 className="font-heading text-3xl md:text-5xl font-black text-primary-foreground mb-3">
                    Poços e{' '}
                    <span className="text-gradient">Subaquático</span>
                  </h1>
                  <p className="font-body text-base text-primary-foreground/60 max-w-2xl">
                    Câmeras submersíveis de alto desempenho para inspeção de poços artesianos, reservatórios e ambientes subaquáticos. Certificação IP68 com cabos de até 100 metros.
                  </p>
                </div>
                <Button
                  variant="whatsapp"
                  ...
                  onClick={() => { openWhatsApp('linha-p'); window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'Botao_WhatsApp_P' }); }}
                >

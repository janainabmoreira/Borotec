...
type FilterKey = keyof Omit<EspeciaisProduct, 'id' | 'name' | 'description' | 'image' | 'specLabels'>;
type FilterState = Record<FilterKey, string[]>;

// ── Configuração dos filtros ──────────────────────────────────────────────────

const filterConfig: { key: FilterKey; label: string; options: string[] }[] = [
  { key: 'probe',  label: 'Diâmetro da sonda', options: ['Ø8mm']      },
  { key: 'camera', label: 'Resolução',           options: ['3D Full HD'] },
...
const ProductCard = ({ product }: { product: EspeciaisProduct }) => {
  ...
  return (
    <div className="group bg-card rounded-2xl overflow-hidden border border-border shadow-card hover:border-accent/40 hover:-translate-y-1 transition-all duration-300">
      <Link to={`/linha-e/${product.id}`} className="block relative overflow-hidden aspect-[4/3] bg-secondary">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
        ...
        <div className="absolute top-3 left-3">
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-yellow-500/90 backdrop-blur-sm text-white text-[10px] font-bold font-heading rounded-full uppercase tracking-wide">
            <IconEspecial className="w-3 h-3" /> Linha E
          </span>
        </div>
      </Link>

      <div className="p-5">
        <Link to={`/linha-e/${product.id}`}>
...
const LineEspeciais = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Videoscópios para Aplicações Especiais | Linha E | BOROTEC Industrial</title>
        <meta name="description" content="Videoscópios de aplicação especial com tecnologia 3D estéreo para medição dimensional sem contato. Soluções avançadas para inspeções críticas industriais." />
        <link rel="canonical" href="https://borotec.com.br/linha-e" />
        <meta property="og:title" content="Videoscópios para Aplicações Especiais | BOROTEC Industrial" />
        <meta property="og:description" content="Tecnologia 3D estéreo para medição dimensional em inspeções industriais críticas." />
        <meta property="og:url" content="https://borotec.com.br/linha-e" />
          ...
          <section className="relative bg-charcoal py-12 md:py-20 overflow-hidden">
            ...
            <div className="relative container-wide mx-auto px-4 md:px-8">
              <nav className="flex items-center gap-2 text-sm text-primary-foreground/50 mb-6">
                ...
                <span>/</span>
                <span className="text-cyan">Aplicações Especiais</span>
              </nav>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-cyan/20 rounded-xl flex items-center justify-center">
                      <IconEspecial className="w-5 h-5 text-cyan" />
                    </div>
                    <span className="inline-flex items-center px-3 py-1 bg-yellow-500/15 text-yellow-400 text-xs font-semibold font-body rounded-full">
                      Linha E
                    </span>
                  </div>
                  <h1 className="font-heading text-3xl md:text-5xl font-black text-primary-foreground mb-3">
                    Aplicações{' '}
                    <span className="text-gradient">Especiais</span>
                  </h1>
                  <p className="font-body text-base text-primary-foreground/60 max-w-2xl">
                    Videoscópios com tecnologias exclusivas para aplicações críticas e especializadas. Câmera 3D estéreo com medição dimensional sem contato para inspeções de alta precisão.
                  </p>
                </div>
                <Button
                  variant="whatsapp"
                  ...
                  onClick={() => { openWhatsApp('linha-e'); window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'Botao_WhatsApp_E' }); }}
                >

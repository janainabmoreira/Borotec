...
import {
  Camera, Droplets, Plug2,
  ChevronDown, ChevronUp, MessageCircle, SlidersHorizontal, X, Loader2,
} from 'lucide-react';
import { IconHospital } from '@/components/LineIcons';
...
type FilterKey = keyof Omit<HospitalarProduct, 'id' | 'name' | 'description' | 'image' | 'specLabels'>;
type FilterState = Record<FilterKey, string[]>;

// ── Configuração dos filtros ──────────────────────────────────────────────────

const filterConfig: { key: FilterKey; label: string; options: string[] }[] = [
  { key: 'probe',  label: 'Diâm. da sonda', options: ['Ø4mm', 'Ø6mm', 'Ø8mm'] },
  { key: 'cable',  label: 'Cabo máx.',       options: ['1m', '2m', '3m']      },
  { key: 'camera', label: 'Resolução',        options: ['HD', 'Full HD']       },
  { key: 'ip',     label: 'Proteção IP',      options: ['IP67', 'IP68']        },
];
...
const ProductCard = ({ product }: { product: HospitalarProduct }) => {
  ...
  return (
    <div className="group bg-card rounded-2xl overflow-hidden border border-border shadow-card hover:border-accent/40 hover:-translate-y-1 transition-all duration-300">
      <Link to={`/linha-h/${product.id}`} className="block relative overflow-hidden aspect-[4/3] bg-secondary">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
        ...
        <div className="absolute top-3 left-3">
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-500/90 backdrop-blur-sm text-white text-[10px] font-bold font-heading rounded-full uppercase tracking-wide">
            <IconHospital className="w-3 h-3" /> Linha H
          </span>
        </div>
      </Link>

      <div className="p-5">
        <Link to={`/linha-h/${product.id}`}>
        ...
        <div className="flex gap-2">
          <Button variant="cta" size="sm" className="flex-1 text-xs" asChild>
            <Link to={`/linha-h/${product.id}`}>Ver detalhes</Link>
          </Button>
        </div>
      </div>
    </div>
...
const LineHospitalar = () => {
  const { openWhatsApp } = useWhatsAppMessage();
  const { products, loading } = useLineProducts('Linha H - Hospitalar');
  ...
  return (
    <>
      <Helmet>
        <title>Boroscópios Hospitalar | Linha H | BOROTEC Industrial</title>
        <meta name="description" content="Boroscópios para inspeção em equipamentos hospitalares, autoclaves, tubulações de gases medicinais e manutenção de equipamentos médicos e veterinários." />
        <link rel="canonical" href="https://borotec.com.br/linha-h" />
        <meta property="og:title" content="Boroscópios para Hospitalar | BOROTEC Industrial" />
        <meta property="og:description" content="Equipamentos para inspeção de equipamentos hospitalares, autoclaves e tubulações de gases medicinais." />
        <meta property="og:url" content="https://borotec.com.br/linha-h" />
          ...
          <section className="relative bg-charcoal py-12 md:py-20 overflow-hidden">
            ...
            <div className="relative container-wide mx-auto px-4 md:px-8">
              <nav className="flex items-center gap-2 text-sm text-primary-foreground/50 mb-6">
                ...
                <span>/</span>
                <span className="text-cyan">Hospitalar</span>
              </nav>
                ...
                <div>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 bg-cyan/20 rounded-xl flex items-center justify-center">
                      <IconHospital className="w-5 h-5 text-cyan" />
                    </div>
                    <span className="inline-flex items-center px-3 py-1 bg-emerald-500/15 text-emerald-400 text-xs font-semibold font-body rounded-full">
                      Linha H
                    </span>
                  </div>
                  <h1 className="font-heading text-3xl md:text-5xl font-black text-primary-foreground mb-3">
                    <span className="text-gradient">Hospitalar</span>
                  </h1>
                  <p className="font-body text-base text-primary-foreground/60 max-w-2xl">
                    Videolaringoscópios, boroscópios e câmeras flexíveis para procedimentos clínicos, intubação de vias aéreas, inspeção de equipamentos hospitalares e manutenção médica.
                  </p>
                </div>
                <Button
                  variant="whatsapp"
                  ...
                  onClick={() => { openWhatsApp('linha-h'); window.dataLayer = window.dataLayer || []; window.dataLayer.push({ event: 'Botao_WhatsApp_H' }); }}
                >
